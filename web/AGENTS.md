...
- **Stack**: Astro + Qwik via `@qwikdev/astro`, Supabase for auth, minimal custom styling in `src/layouts/RootLayout.astro`. Auth pages share `src/components/AuthForm.astro`.
...
- **Styling/layout**: Global CSS lives in `src/layouts/RootLayout.astro`; auth shell in `src/components/AuthForm.astro`; components are plain and reuse shared classes (`card`, `cta`, etc.).
